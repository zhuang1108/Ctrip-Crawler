...
def SetHeadersAndParams(dcitycode, acitycode, searchdate = '2018-08-25'):
	dcityname = citycode[dcitycode]
	acityname = citycode[acitycode]
	dcityid = cityid[dcityname]
	acityid = cityid[acityname]
...
# 根据请求头获取json数据，并判断通航情况
def GetJsonData(dcitycode, acitycode, searchdate = '2018-08-25'):

	headers, data = SetHeadersAndParams(dcitycode, acitycode, searchdate)
...
def SaveDataToMysql(jsonData):
	'''
	# 两城市所有基本信息，城市id，name，airport，airline等
	cityNames = jsonData['data']['basicModelData']['cityNames']
	cityIDs = jsonData['data']['basicModelData']['cityIDs']
	airlineNames = jsonData['data']['basicModelData']['airlineNames']
	airportNames = jsonData['data']['basicModelData']['airportNames']
	terminalNames = jsonData['data']['basicModelData']['terminalNames']
	craftTypes = jsonData['data']['basicModelData']['craftTypes']
	'''
	...
	print('This json data package has been managed.')

# managerman
def ManagerFunction(searchdate):
		
	print('.....Start The Robot Now.....')
	for dcitycode, dcityname in sorted(citycode.items()):
		for acitycode, acityname in sorted(citycode.items()):
			if dcitycode != acitycode:
				print('\n正在处理 %s 到 %s 的机票信息...' % (dcityname, acityname))
	...
	print('.....Congratulations to you!!! You have got all flights.....')


if __name__ == '__main__':
	try:
		ManagerFunction('2018-08-15')
		# jsonData = GetJsonData('BJS', 'CKG', '2018-08-15')
		# SaveDataToMysql(jsonData)
	finally:
		cur.close()
		conn.close()
